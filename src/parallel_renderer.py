...
def _render_frame_batch(args):
    ...
    visualizer_module, visualizer_class_name, config_dict, features_dict, frame_indices = args
...
class ParallelRenderer:
    ...
    def render_frames_parallel(
        ...
        logger.info(f"Paralleles Rendering: {len(frame_indices)} Frames in {len(batches)} Batches")
            ...
            for future in as_completed(future_to_batch):
                ...
                try:
                    ...
                    if completed % 50 == 0:
                        ...
                        logger.info(f"Fortschritt: {progress:.1f}% ({completed}/{len(frame_indices)})")
...
class StreamingParallelRenderer(ParallelRenderer):
    ...
    def render_to_file(
        ...
        logger.info(f"Streaming-Rendering: {len(frame_indices)} Frames in {len(batches)} Batches")
    ...
    def _render_batch_to_temp(
        ...
        temp_file = tempfile.NamedTemporaryFile(
            suffix=f'_batch_{batch_idx}.raw',
            delete=False
        )
        temp_file.close()
