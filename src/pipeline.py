...
from .parallel_renderer import get_optimal_workers
...
class RenderPipeline:
    ...
    def run(
        ...
        features = self.analyzer.analyze(str(audio_path), fps=self.config.visual.fps)
    ...
    def _render_video(
        ...
        try:
            ...
            temp_video = tempfile.NamedTemporaryFile(
                suffix=".mp4", delete=False, dir=str(settings.temp_dir)
            )
            temp_video.close()
            ...
            logger.info(
                f"Starte Rendering ({features.frame_count} Frames @ {fps}fps)..."
            )
            if self.parallel:
                logger.info(
                    f"[EXPERIMENTAL] Paralleles Rendering mit {self.num_workers} Workern"
                )
            if progress_callback:
                progress_callback(
                    0.0, f"Starte Rendering ({features.frame_count} Frames)..."
                )
            ...
            # OPTIMIERUNG: Frame-Buffering für bessere Performance
            # Reduziert System-Calls und erhöht Durchsatz
            FRAME_BUFFER_SIZE = 10
            frame_buffer = bytearray()
            frame_size = width * height * 3

            # Frame-Loop mit Fortschrittsanzeige und Error-Handling
            for i in range(features.frame_count):
                try:
                    ...
                    # OPTIMIERUNG: Buffer statt einzelnes Write
                    frame_buffer.extend(frame.tobytes())

                    # Schreibe wenn Buffer voll oder letzter Frame
                    if (
                        len(frame_buffer) >= FRAME_BUFFER_SIZE * frame_size
                        or i == features.frame_count - 1
                    ):
                        process.stdin.write(frame_buffer)
                        frame_buffer = bytearray()
                    ...
                    if i % 30 == 0 or i == features.frame_count - 1:
                        ...
                        logger.info(
                            f"Rendering: {progress_pct:.1f}% ({i+1}/{features.frame_count})"
                        )
                        if progress_callback:
                            progress_callback(
                                progress, f"Rendering... {progress_pct:.1f}%"
                            )

                except Exception as e:
                    logger.exception(f"Fehler beim Rendern von Frame {i}: {e}")
                    # Schreibe einen schwarzen Frame als Platzhalter
                    black_frame = np.zeros((height, width, 3), dtype=np.uint8)
                    frame_buffer.extend(black_frame.tobytes())
            ...
            # Schließe stdin und warte auf FFmpeg (mit Timeout)
            process.stdin.close()
            process.wait(timeout=7200)  # 2 Stunden Timeout für lange Videos
            ...
            if temp_video is not None and Path(temp_video.name).exists():
                try:
