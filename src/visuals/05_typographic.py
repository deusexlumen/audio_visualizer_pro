...
class TypographicVisualizer(BaseVisualizer):
    ...
    def setup(self):
        ...
        try:
            self.title_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 72)
            self.subtitle_font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 36)
        except:
    ...
    def render_frame(self, frame_idx: int) -> np.ndarray:
        ...
        for i in range(num_bars):
            ...
            if i < num_bars // 2:
                ratio = i / (num_bars // 2)
                color = tuple(int(primary[j] * ratio + secondary[j] * (1 - ratio)) for j in range(3))
            else:
                ratio = (i - num_bars // 2) / (num_bars // 2)
                color = tuple(int(secondary[j] * ratio + primary[j] * (1 - ratio)) for j in range(3))
        ...
        draw.line([(0, self.waveform_y), (self.width, self.waveform_y)], 
        ...
        if onset > 0.3:
            ...
            draw.ellipse(
                [self.width // 2 - indicator_radius, self.waveform_y - indicator_radius,
                 self.width // 2 + indicator_radius, self.waveform_y + indicator_radius],
                outline=primary,
                width=3
            )
        ...
        draw.rectangle(
            [bar_x, bar_y, bar_x + progress_width, bar_y + 6],
            fill=primary
        )
        ...
        try:
            draw.text((bar_x, bar_y - 30), time_text, fill=(200, 200, 200), font=self.subtitle_font)
        except:
        ...
        try:
            draw.text((bar_x, 30), mode_text, fill=(150, 150, 150), font=self.subtitle_font)
        except:
