...
class SacredMandalaVisualizer(BaseVisualizer):
    ...
    def render_frame(self, frame_idx: int) -> np.ndarray:
        ...
        self._draw_flower_of_life(
            draw, self.center, self.base_radius, colors[dominant_note], rms, chroma
        )

        # 2. Sechseck-Muster
        hex_color = colors[(dominant_note + 2) % 12]
        self._draw_polygon_ring(
            draw, self.center, self.base_radius * 0.75, 6, hex_color, self.rotation, rms
        )
        ...
        draw.ellipse(
            [
                self.center[0] - pulse_radius,
                self.center[1] - pulse_radius,
                self.center[0] + pulse_radius,
                self.center[1] + pulse_radius,
            ],
        ...
        if onset > 0.3:
            self._draw_connection_lines(
                draw, self.center, self.base_radius, colors[dominant_note], onset
            )

        # Partikel/Spuren bei hohem RMS
        if rms > 0.6:
            self._draw_energy_particles(
                draw, self.center, self.base_radius, colors, rms, frame_idx
            )
    ...
    def _draw_flower_of_life(
        ...
        for i in range(num_circles):
            ...
            rgb = colorsys.hsv_to_rgb(
                hue, 0.7 + chroma_strength * 0.3, 0.5 + chroma_strength * 0.5
            )
    ...
    def _draw_connection_lines(
        ...
        for i in range(num_lines):
            ...
            draw.line(
                [(x1, y1), (x2, y2)], fill=color, width=max(1, int(intensity * 4))
            )

    def _draw_energy_particles(
        self,
        draw: ImageDraw.Draw,
        center: tuple,
        radius: float,
