...
class NeonWaveCircleVisualizer(BaseVisualizer):
    ...
    def render_frame(self, frame_idx: int) -> np.ndarray:
        ...
        for i in range(self.num_rings - 1, -1, -1):
            ...
            if i % 2 == 0:
                ring_color = tuple(
                    int(primary_color[j] * (0.5 + ring_val * 0.5)) for j in range(3)
                )
            else:
                ring_color = tuple(
                    int(secondary_color[j] * (0.5 + ring_val * 0.5)) for j in range(3)
                )

            # Zeichne den wellenförmigen Ring
            self._draw_wavy_ring(
                draw,
                self.center,
                current_radius,
                current_amplitude,
        ...
        draw.ellipse(
            [
                self.center[0] - core_radius,
                self.center[1] - core_radius,
                self.center[0] + core_radius,
                self.center[1] + core_radius,
            ],
            fill=core_color,
        ...
        draw.ellipse(
            [
                self.center[0] - inner_glow_radius,
                self.center[1] - inner_glow_radius,
                self.center[0] + inner_glow_radius,
                self.center[1] + inner_glow_radius,
            ],
        ...
        if onset > 0.4:
            self._draw_particle_rays(
                draw, self.center, self.max_radius, primary_color, onset, rms, time
            )

        # Frequenz-Balken als äußerer Ring (simuliert)
        self._draw_frequency_ring(
            draw, self.center, self.max_radius + 20, chroma, time, rms
        )
    ...
    def _draw_particle_rays(
        ...
        for i in range(num_rays):
            ...
            for j in range(3):
                offset_angle = j * 0.02
                ox = center[0] + math.cos(angle + offset_angle) * (
                    max_radius + ray_length * 0.8
                )
                oy = center[1] + math.sin(angle + offset_angle) * (
                    max_radius + ray_length * 0.8
                )
    ...
    def _draw_frequency_ring(
        ...
        for i in range(num_bars):
            ...
            draw.line(
                [(bar_x, bar_y), (bar_end_x, bar_end_y)],
                fill=bar_color,
                width=bar_width,
            )
