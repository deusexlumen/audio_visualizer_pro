...
class NeonOscilloscopeVisualizer(BaseVisualizer):
    ...
    def render_frame(self, frame_idx: int) -> np.ndarray:
        ...
        for glow_offset in [6, 10, 14]:
            ...
            self._draw_waveform(draw, waveform, glow_color, width=glow_offset, offset=glow_offset//4)
        ...
        draw.rectangle([2, 2, self.width-3, self.height-3], outline=border_color, width=2)
        
        # UI-Elemente (Scan-Linie)
        scan_x = int((frame_idx % self.width))
        draw.line([(scan_x, 0), (scan_x, self.height)], fill=(neon_color[0]//4, neon_color[1]//4, neon_color[2]//4), width=2)
    ...
    def _draw_waveform(self, draw: ImageDraw.Draw, points: list, color: tuple, width: int = 2, offset: int = 0):
